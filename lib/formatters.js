...
module.exports = {
  ...
  toJSFileName: function (name) {
    return '' + changeCase.snake(name) + '.js';
  },
  ...
  toPartialControllerName: function (name) {
    return '_' + changeCase.snake(name) + '_controller.js';
  },
  ...
  checkName: function (type, name) {
    ...
    if (name.match(/^[^a-zA-Z]/) || name.match(/[^-_a-zA-Z0-9/]/)) {
      return logger.warn("You can't use '" + name.blue + ("' as a " + type + " name.").yellow);
    }
    ...
    switch (name.toLowerCase()) {
      ...
      case 'n':
      case 'cancel':
        return logger.warn('Any ' + type + ' should have a name');
        break;
    }

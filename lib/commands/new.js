...
module.exports = function () {
    ...
    try {
        console.log('Checking for bower installation...');
        child_process.execSync('npm list -g bower');

    } catch (e) {
        logger.warn('bower must be installed globally.');
        console.log('Would you like to install bower now?');

        var result = prompt('Yes'.green + ' or ' + 'No: '.red).toLowerCase();

        if (result == 'yes') {
            console.log('Installing bower...');
            child_process.execSync('npm install -g bower');
            logger.done();
        }

        if (result == 'no') {
            console.log('Please install bower and run kick again.');
            return;
        }
    }

...
function directive(name, args, opts) {
  ...
  args.map(function (otherName) { generate.directive(otherName, template); });
  logger.done();
}
...
function partial(name, args, opts) {
  ...
  args.map(function (otherName) { generate.partial(otherName, controller); });
  logger.done();
}

function generateType(type, name, args) {
  generate[type](name);
  args.map(function (otherName) { generate[type](otherName); });
  logger.done();
}

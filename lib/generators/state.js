...
module.exports = function (name, noController, abstract) {
  ...
  if (!Utils.exists('app/config/routes/' + Format.toJSFileName(rootState))) {
    createFile('app/config/routes/' + Format.toJSFileName(rootState), Templates.stateRoute({
      stateName: rootState
    }));
    ...
    prependToFile('app/config/routes/routes.js', "import { " + rootState + "Routes } from './" + Format.toFolderName(rootState) + "';\n");
